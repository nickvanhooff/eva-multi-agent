                      ...
                      <tr key={key} style={{ borderTop: '1px solid var(--border)' }}>
                        ...
                        <td style={{ padding: '14px 12px 14px 0' }}>
                          <select
                            ...
                            style={inputStyle}
                            aria-label={`${AGENT_LABELS[key] || key} provider`}
                          >
                            {providers.map((p) => (
                        ...
                        <td style={{ padding: '14px 12px 14px 0' }}>
                          <select
                            ...
                            style={inputStyle}
                            aria-label={`${AGENT_LABELS[key] || key} model`}
                          >
                            {recommended.length > 0 && (
                              <optgroup label="Recommended (saved / default)">
                                {recommended.map((m) => (
                        ...
                        <td style={{ padding: '14px 0' }}>
                          <input
                            ...
                            onChange={(e) => updateAgent(key, 'temperature', Number.parseFloat(e.target.value))}
                            style={{ width: '100%' }}
                            aria-label={`${AGENT_LABELS[key] || key} temperature`}
                          />
                          <span style={{ color: 'var(--text-muted)', fontSize: 12, marginLeft: 8 }}>
                            {(row.temperature ?? 0.7).toFixed(1)}
                          </span>
                        </td>
                      </tr>
                    )
